...
# 替换为国内源
replace_with_cn_mirrors() {
    log "info" "替换为国内镜像源，加速下载..."
...
gpgcheck=1
gpgkey=http://mirrors.aliyun.com/centos-vault/RPM-GPG-KEY-CentOS-Official
...
timeout = 120
EOF
    check_result $? "pip镜像配置完成"
    
    log "success" "国内镜像源基础配置完成"
}

# 配置npm国内镜像（容错版）
set_npm_mirror() {
    log "info" "配置npm国内镜像（淘宝源）..."
    if npm config set registry https://registry.npmmirror.com/ > /dev/null 2>&1 && npm config set disturl https://npmmirror.com/dist > /dev/null 2>&1; then
        log "success" "npm镜像配置完成"
    else
        log "warning" "npm镜像配置失败，将使用默认源，前端依赖安装可能较慢"
    fi
}
...
main() {
    ...
    # 安装Node.js 20（使用国内源，采用二进制包安装方式，满足前端依赖要求）
    log "info" "安装Node.js 20（使用国内源）..."
    
    # 使用Node.js官网国内镜像下载二进制包
    NODE_VERSION="20.19.0"
    NODE_BINARY_URL="https://npmmirror.com/mirrors/node/v$NODE_VERSION/node-v$NODE_VERSION-linux-x64.tar.xz"
    ...
    # 安装PostgreSQL 13（使用国内源）
    log "info" "安装PostgreSQL 13（使用国内源）..."
    yum install -y https://mirrors.aliyun.com/postgresql/repos/yum/reporpms/EL-8-x86_64/pgdg-redhat-repo-latest.noarch.rpm > /dev/null 2>&1
    ...
    # 配置Git国内镜像（Gitee）
    git config --global url.https://gitee.com/.insteadOf https://github.com/ > /dev/null 2>&1
    check_result $? "Git镜像配置完成"
...
EOF
    ...
    # 安装依赖（使用国内源，并输出详细日志）
    log "info" "正在安装前端依赖..."
    if npm install --registry=https://registry.npmmirror.com/ --disturl=https://npmmirror.com/dist --sass-binary-site=https://npmmirror.com/mirrors/node-sass/; then
        log "success" "前端依赖安装完成"
    else
        log "warning" "前端依赖安装失败，尝试使用默认源安装..."
        npm install
        if [ $? -eq 0 ]; then
